...
namespace DENETWORKLINGS
{
    ...
    {
        public static T[] TrimLeft<T>(this T[] aOriginal, int aSize)
        {
            T[] toReturn = new T[aOriginal.Length - aSize];
            aOriginal.CopyTo(toReturn, aSize);
        ...
        private void DataReceived(IAsyncResult ar)
        {
            ...
            while (exitWhile)
            {
                ...
                {
                    _objectDataLength = BitConverter.ToInt32(_incomingData, 0);
                    _incomingData = _incomingData.Skip(IntSize).ToArray();
                    //_incomingData = TrimLeft(_incomingData, IntSize);
